...
export async function POST(req: NextRequest) {
  try {
    ...
    const newItem = await prisma.equipment.create({
      data: {
        name: body.name,
        serialNumber: body.isIndividual ? body.serialNumber : null, // optional สำหรับ quantity-based
        category: body.category,
        description: body.description || "",
        total: body.isIndividual ? 0 : body.total, // 0 สำหรับ individual
        ...
        availableQuantity: body.isIndividual ? 0 : body.total, // 0 สำหรับ individual
        brokenQuantity: 0,
        lostQuantity: 0,
        inUseQuantity: 0,
      },
